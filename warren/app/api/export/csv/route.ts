  ...
  try {
    ...
    try {
      ...
      if (dataResponse.ok) {
        ...
        console.log('✅ Successfully fetched dashboard data for CSV export');
      } else {
        console.warn('⚠️ Failed to fetch dashboard data, proceeding with empty export');
      }
    } catch (error) {
      console.error('❌ Error fetching dashboard data for CSV export:', error);
    ...
    // Transform dashboard data for CSV export
    const exportData = {
    ...
    // Generate CSV
    const csvBuffer = await exportService.generateCsvExport(exportData);
