  ...
  try {
    ...
    try {
      ...
      if (dataResponse.ok) {
        ...
        console.log('✅ Successfully fetched dashboard data for JSON export');
      } else {
        console.warn('⚠️ Failed to fetch dashboard data, proceeding with empty export');
      }
    } catch (error) {
      console.error('❌ Error fetching dashboard data for JSON export:', error);
    ...
    // Transform dashboard data for JSON export
    const exportData = {
    ...
    const exportService = new JsonExportService();

    // Generate JSON
    const jsonBuffer = await exportService.generateJsonExport(exportData);
