// Warren database schema - uses warren's own drizzle-orm instance
...
export const configurationsTable = pgTable("company_configurations", {
  id: uuid("id").primaryKey().defaultRandom(),
  companyId: uuid("company_id").references(() => companies.id).notNull(),
...
export const processedFinancialData = pgTable("processed_financial_data", {
  id: uuid("id").primaryKey().defaultRandom(),
  companyId: uuid("company_id").references(() => companies.id).notNull(),
  configId: uuid("config_id").references(() => configurationsTable.id).notNull(),
...
// Export new types
export type CompanyConfiguration = typeof configurationsTable.$inferSelect;
export type NewCompanyConfiguration = typeof configurationsTable.$inferInsert;
