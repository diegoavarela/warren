...
interface OperatingExpensesDetailModalProps {
  isOpen: boolean;
  onClose: () => void;
  categoryData: {
    category: string;
...
export function OperatingExpensesDetailModal({
  isOpen,
  onClose,
  categoryData,
  currency = '$',
  formatValue
}: OperatingExpensesDetailModalProps) {
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-blue-100 text-sm">
                {currency} {valueFormatter(categoryData.amount)} ({categoryData.percentage.toFixed(1)}% of total OpEx)
              </p>
              {categoryData.items.some(item => item.subcategory) && (
                <p className="text-blue-200 text-xs mt-1">
                  Grouped from multiple QuickBooks categories
                </p>
              )}
            </div>
            <button
                    ...
                    <div className="flex-1">
                      ...
                      {item.subcategory && (
                        <p className="text-xs text-gray-500">
                          Original category: {item.subcategory}
                        </p>
                      )}
                    </div>
              ...
              <div className="bg-gray-100 rounded-lg p-4">
                <div className="grid grid-cols-3 gap-4 text-center">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-600">Of Total OpEx</div>
                  </div>
                </div>
              </div>
            </div>
...
export default OperatingExpensesDetailModal;