...
interface QuickBooksNetIncomeTrendChartProps {
  data: TrendDataPoint[];
  title: string;
  currency: string;
...
export function QuickBooksNetIncomeTrendChartJS({
  data,
  title,
  currency,
  formatValue
}: QuickBooksNetIncomeTrendChartProps) {
  ...
  const { chartData, forecastStats } = useMemo(() => {
    ...
    const lowerBand = allPoints.map((point, index) => {
      const variance = standardError * confidenceMultiplier * Math.sqrt(1 + Math.pow(point.x - actualData.length/2, 2) / actualData.length);
      return point.y - variance; // Allow negative values for net income
    });
    ...
    const currentTrend = actualData.length > 1
      ? ((actualData[actualData.length - 1].y - actualData[actualData.length - 2].y) / Math.abs(actualData[actualData.length - 2].y) * 100)
      : 0;
  ...
  const options: ChartOptions<'line'> = {
    ...
    scales: {
      ...
      y: {
        beginAtZero: false, // Allow negative values for net income
        position: 'left',
        ticks: {
        ...
        <div className="mt-6 grid grid-cols-3 gap-4 flex-shrink-0">
          ...
          <div className="bg-blue-50 rounded-lg p-4">
            ...
            <p className="text-lg font-semibold text-blue-900">
              ±{((forecastStats?.confidenceRange || 0) / 2 / Math.abs(forecastStats?.sixMonthForecast || 1) * 100).toFixed(1)}%
            </p>
          </div>
        </div>
      </div>
