...
import { users, user2faSettings, user2faAttempts } from '@/lib/db';
import { eq, and, gte } from 'drizzle-orm';
import { requireAuth } from '@/lib/auth-middleware';
import { verifyTOTP, verifyBackupCode, removeUsedBackupCode } from '@/lib/totp-utils';

// POST /api/users/[id]/2fa/toggle - Enable or disable 2FA for a user
export const POST = requireAuth(async (request: NextRequest, adminUser, { params }: { params: { id: string } }) => {
  try {
    const userId = params.id;
    const { token, action } = await request.json(); // action: 'enable' or 'disable'

    if (!token || !action || !['enable', 'disable'].includes(action)) {
      return NextResponse.json(
        { success: false, error: 'Token and valid action (enable/disable) are required' },
        { status: 400 }
      );
    }
    ...
    if (!existingUser.isActive) {
      return NextResponse.json(
        { success: false, error: 'Cannot modify 2FA for inactive user' },
        { status: 400 }
      );
    }
    ...
    // Update 2FA settings based on action
    ...
    return NextResponse.json({
      success: true,
      message: `2FA ${action}d successfully for ${existingUser.firstName} ${existingUser.lastName}`,
      data: {
        userId: existingUser.id,
        email: existingUser.email,
        twoFactorEnabled: newEnabledState,
